...
class ExpansivePath(nn.Module):
    def __init__(self, in_channels, out_channels, mid_channels=None):
        super().__init__()
        self.upconv = upconv(in_channels)
...
class UNet(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        self.input_conv = DoubleConv(in_channels, 2 ** 6, kernel_size=3, stride=1, padding=1)
        self.left = nn.ModuleList([ContractingPath(2 ** i, 2 ** (i+1)) for i in range(6, 11)])
        self.right = nn.ModuleList([ExpansivePath(2 ** i, 2 ** (i-1)) for i in range(10, 5, -1)])
